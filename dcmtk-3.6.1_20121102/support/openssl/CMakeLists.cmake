...
IF(NOT PERL OR NOT NASM)
  MESSAGE(SEND_ERROR "PERL or NASM does not install.")
ENDIF()
ENABLE_TESTING()

SET(ASM_LIBRARY "asmlib")
...
IF(WIN32)
  IF(BUILD_SHARED_LIBS)
    ADD_DEFINITIONS(-D_WINDLL -DOPENSSL_NO_STATIC_ENGINE)
...
IF(OPENSSL_ASM_SUPPORT)
  ADD_DEFINITIONS(-DNO_ASM -DSHA1_ASM -DSHA256_ASM -DSHA512_ASM -DMD5_ASM
...
-DOPENSSL_BN_ASM_PART_WORDS -DOPENSSL_IA32_SSE2 -DOPENSSL_BN_ASM_MONT -DOPENSSL_BN_ASM_GF2m
  ...
  SET(NASM_CFLAG /Ox /Od /Ob2 -W3 -Gs0 -GF -Gy -nologo)
  SET(NASM_OPTION win32)
  IF(DCMTK_LIBRARY_TYPE EQUAL SHARED)
    SET(NASM_OBJ_FLAG /MD /OPENSSL_BUILD_SHLIBCRYPTO)
  ELSEIF(DCMTK_LIBRARY_TYPE EQUAL STATIC)
    SET(NASM_OBJ_FLAG /MT)
  ENDIF()
ELSE(OPENSSL_ASM_SUPPORT)
  ADD_DEFINITIONS(-DNO_ASM)
ENDIF(OPENSSL_ASM_SUPPORT)
...
IF(WIN32)
  IF(MSVC)
    SET(PERL_PER_Command ${PERL} Configure no-asm VC-WIN32)
    SET(COPY_CMD ${PERL} util/copy.pl)
  ENDIF(MSVC)
  IF(MINGW)
    SET(PERL_PER_Command ${PERL} Configure no-asm mingw)
    SET(COPY_CMD cmake -E copy)
  ENDIF(MINGW)

  ADD_CUSTOM_COMMAND(OUTPUT Perl_Configure
      COMMAND ${PERL_PER_Command}
...
# NASM assembling processes
IF(OPENSSL_ASM_SUPPORT)
  FOREACH(_FILE ${ASM_FILE_LIST})
    GET_FILENAME_COMPONENT(NAME ${_FILE} NAME_WE)
    SET(OUT_ASM ${TMP_D}/${NAME}.asm)
    SET(OUT_OBJ ${TMP_D}/${NAME}.obj)
    ADD_CUSTOM_COMMAND(OUTPUT ${OUT_OBJ}
      COMMAND ${PERL} ${_FILE} ${NASM_OPTION}n ${NASM_OBJ_FLAG} ${COMPILE_DEFINITIONS} >${OUT_ASM}
      COMMAND ${NASM} -f ${NASM_OPTION} -o ${OUT_OBJ} ${OUT_ASM}
      WORKING_DIRECTORY ${OPENSSL_ORIGN_DIR}
      DEPENDS ${OPENSSL_ORIGN_DIR}/${_FILE})
    LIST(APPEND NASM_FILES ${OUT_OBJ})
  ENDFOREACH()
  ADD_CUSTOM_TARGET(NASM_ASM_SCRIPT ALL DEPENDS ${NASM_FILES})
ENDIF(OPENSSL_ASM_SUPPORT)
...
IF(BUILD_SHARED_LIBS)
  IF(OPENSSL_ASM_SUPPORT)
    set_source_files_properties(${NASM_FILES} PROPERTIES GENERATED TRUE)
#    ADD_LIBRARY(${ASM_LIBRARY} SHARED ${NASM_FILES})
    ADD_LIBRARY(${CRYPTO_LIBRARY} SHARED ${CRYPTO_SRCS} ${GOST_SRCS} ${TMPDLL_D}/${CRYPTO_LIBRARY}.def ${NASM_FILES})
  ELSE(OPENSSL_ASM_SUPPORT)
...
ELSE(BUILD_SHARED_LIBS)
  ...
  IF(OPENSSL_ASM_SUPPORT)
    ADD_LIBRARY(${CRYPTO_LIBRARY} STATIC ${CRYPTO_SRCS} ${GOST_SRCS})
  ELSE(OPENSSL_ASM_SUPPORT)
...
ADD_DEPENDENCIES(${CRYPTO_LIBRARY} NASM_ASM_SCRIPT PROPERTIES LINKER_LANGUAGE ASM)
...
IF(NOT SKIP_INSTALL_HEADERS AND NOT SKIP_INSTALL_ALL)
  INSTALL(TARGETS ${CRYPTO_LIBRARY} ${SSL_LIBRARY}
